...
function detectPriceManipulation(message) {
    return PRICE_MANIPULATION_PATTERNS.some(pattern => pattern.test(message));
}
export async function classifyUserIntent(userMessage, projectContext, modelId) {
...
{"type": "no_action", "intent": "general", "payload": {}}
    ...
    try {
        ...
        let judgment = 'GENERAL';
        if (response.includes('RELATED'))
            judgment = 'RELATED';
        else if (response.includes('NEW_PROJECT'))
            judgment = 'NEW_PROJECT';
        else if (response.includes('GENERAL'))
            judgment = 'GENERAL';
...
export function extractProjectContext(modules) {
...
function inferProjectTitle(modules) {
